...
import {
  APP_CONFIG, COURSE_CATALOG, PLAN_CONFIG, REQUIREMENTS,
  ...
  exportCoursesToCsv, getEligibility, gradeToPoint, importCoursesFromCsv, migrateState,
  normalizeCourse, parseBackupPayload, planStats, serializeBackup, upsertCourse,
...
assert.equal(COURSE_CATALOG.find((course) => course.courseCode === "PHYS-1").officialCourseCode, "E815710");
assert.equal(COURSE_CATALOG.find((course) => course.courseCode === "FLUID-1").officialCourseCode, "E827110");

const normalizedPhysics = normalizeCourse({ courseCode: "PHYS-1", courseName: "普通物理學（一）" });
...
const generalCap = [
  complete({ id: "gp", courseCode: "GEN-PHYS", courseName: "應用物理與實驗", credits: 2, requirementGroup: "general", generalSubarea: "naturalEngineering" }),
  complete({ id: "gc", courseCode: "GEN-CHEM", courseName: "應用化學與實驗", credits: 2, requirementGroup: "general", generalSubarea: "naturalEngineering" }),
];
...
const unrelated = [complete({ id: "x", courseCode: "x", courseName: "不認列活動", credits: 3, requirementGroup: "other", countsTowardGraduation: false })];
assert.equal(calculateGraduation(unrelated).total.recognized, 0);

const electives = Array.from({ length: 11 }, (_, index) => complete({ id: `e-${index}`, courseCode: `e-${index}`, courseName: `選修 ${index}`, credits: 3, requirementGroup: "elective", departmentElective: index < 3 }));
...
const riskSummary = calculateRiskSummary([
  complete({ id: "done", courseCode: "done", courseName: "已完成高風險", credits: 3, requirementGroup: "major-core", gpaRisk: "高" }),
  normalizeCourse({ id: "todo", courseCode: "todo", courseName: "未完成高風險", credits: 3, requirementGroup: "major-core", gpaRisk: "高" }),
]);
...
const a = normalizeCourse({ id: "a", courseCode: "a", courseName: "A", credits: 3, requirementGroup: "major-core", term: APP_CONFIG.currentTerm, slots: [{ day: 1, start: 7, end: 8 }] });
const b = normalizeCourse({ id: "b", courseCode: "b", courseName: "B", credits: 3, requirementGroup: "major-core", term: APP_CONFIG.currentTerm, slots: [{ day: 1, start: 8, end: 9 }] });
assert.equal(calculateConflicts([a, b]).length, 1);

const tooSmall = [normalizeCourse({ id: "p1", courseCode: "p1", courseName: "P1", credits: 3, requirementGroup: "general", planA: true, riskScore: 1 })];
assert.equal(planStats(tooSmall, PLAN_CONFIG[0]).valid, false);
const validPlan = Array.from({ length: 4 }, (_, i) => normalizeCourse({ id: `vp-${i}`, courseCode: `vp-${i}`, courseName: `VP${i}`, credits: 3, requirementGroup: "general", planA: true, riskScore: 2, necessityScore: 3 }));
assert.equal(planStats(validPlan, PLAN_CONFIG[0]).valid, true);

const candidate = normalizeCourse({ id: "candidate", courseCode: "candidate", courseName: "候選", credits: 3, requirementGroup: "major-core", term: APP_CONFIG.currentTerm, decision: "priority" });
...
crud = upsertCourse(crud, { id: "custom-ai", courseCode: "CUSTOM-AI", courseName: "AI 應用", credits: 3, recognizedCredits: 3, requirementGroup: "elective", countsTowardGraduation: true, departmentElective: true });
assert.equal(crud.some((course) => course.id === "custom-ai"), true);
crud = upsertCourse(crud, { ...crud.find((course) => course.id === "custom-ai"), courseName: "AI 應用實作" });
...
assert.equal(validateCourseCollection([{ id: "x", courseCode: "x", courseName: "A" }, { id: "x", courseCode: "y", courseName: "B" }]).valid, false);
...
const gpa = calculateGpaProjection({ currentGpa: 3, currentCredits: 30, courses: [normalizeCourse({ id: "g1", courseCode: "g1", courseName: "GPA 課", credits: 3, requirementGroup: "elective", expectedGrade: "90" })] });
assert.equal(gpa.semesterGpa, 4.3);
assert.equal(gpa.semesterCredits, 3);
assert.equal(gpa.projectedGpa, 3.12);

const csvSource = [normalizeCourse({ id: "csv-1", courseCode: "CSV-1", courseName: "CSV 課程", credits: 2, recognizedCredits: 2, requirementGroup: "elective", departmentElective: true })];
...
assert.match(exportCoursesToCsv([normalizeCourse({ id: "bad", courseCode: "bad", courseName: "=HYPERLINK(\"bad\")", credits: 1, requirementGroup: "elective" })]), /'=HYPERLINK/);
